...
// Initialize and attach event listeners after DOM is ready
document.addEventListener('DOMContentLoaded', async () => {
    await initDemoData(); // ensure DB is ready
    ...
    loginBtn.onclick = async () => {
        ...
        if (!success) loginMessage.innerText = 'بيانات غير صحيحة';
        else loginMessage.innerText = '';
    };
    ...
    registerBtn.onclick = async () => {
        ...
        const pass = loginPassword.value.trim();
        if (!email || !pass) {
            loginMessage.innerText = 'يرجى إدخال البريد وكلمة المرور';
            return;
        }
        const success = await register(email, pass, email.split('@')[0]);
        if (!success) loginMessage.innerText = 'البريد موجود مسبقاً';
        else loginMessage.innerText = '';
    };
