...
window.handleBackButton = function() {
    ...
    // الصفحة الرئيسية: عرض تأكيد الخروج
...
async function showMedDetails(med) {
    ...
    // ترتيب الحقول حسب الصورة: الاسم العلمي، الاسم التجاري، المنشأ، الشركة، الشكل الدوائي، التصنيف، تاريخ الانتهاء (إذا لم تكن صفحة كل الأدوية)، الجرعة، الباركود
    detailDiv.innerHTML = `
    ...
    if (currentPage !== 'all') {
        detailDiv.insertAdjacentHTML('beforeend', `
            <div class="med-detail-item"><div class="med-detail-label">${t('expiry_date')}</div><div class="med-detail-value">${escapeHtml(med.expiry)}</div></div>
        `);
    }
    ...
    if (deleteBtn) deleteBtn.onclick = () => { if (confirm(t('delete_confirm'))) moveToDeleted(med.id); closeModal('medModal'); };
    openModal('medModal');
}
